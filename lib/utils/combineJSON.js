...
async function combineJSON(arr, deep, collision, source, parsers=[]) {
  ...
  for (i = 0; i < arr.length; i++) {
    var new_files = await asyncGlob(arr[i], { fs });
  ...
  for (i = 0; i < files.length; i++) {
    ...
    try {
      ...
      await Promise.all(
        parsers.map(({ pattern, parse }) => {
          return new Promise(async (resolve) => {
            if (resolvedPath.match(pattern)) {
              const contents = await new Promise((resolve) => {
                fs.readFile(resolvedPath, {encoding: 'UTF-8'}, (err, data) => {
                  resolve(data);
                });
              });
              file_content = parse({
      ...
      if (!file_content) {
        ...
        file_content = deepExtend([file_content, content]);
      }
    } catch (e) {
